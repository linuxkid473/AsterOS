...
/* AsterOS addition: the separate, deprecated *userspace* OSAtomic*
 * naming convention (OSAtomicAdd64/OSAtomicIncrement32/etc, verb
 * *after* "OSAtomic" -- distinct from this kernel header's own
 * OSAddAtomic/OSIncrementAtomic names above). Real Darwin provided
 * these from a separate userspace-only Libc/libplatform project we
 * don't have vendored; ld64's InputFiles.cpp is the only caller in
 * this tree, using them just to keep running byte/file counters, so
 * plain __sync builtins are correct and sufficient. */
